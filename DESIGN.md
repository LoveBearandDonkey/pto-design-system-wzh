...
For memory hierarchy diagrams, reuse pattern id `memory-architecture-layout`. The canonical source is the hybrid renderer in `patterns/memory-architecture/pattern.js`, extracted from `mem_viewer` DOM, BPG grid logic, MTE overlay behavior, hardware hover tips, and `ascend-hardware-map-v3` path-focus hover. New hardware pages such as 950B should extend the preset/config surface there and call `attachPathFocusInteractions` or `setPathFocus` / `clearPathFocus` for node and flow-step hover instead of copying `mem_viewer/index.html`, redrawing route geometry, or reimplementing `is-path-focused` / route glow locally.
